...
HRESULT CComponentManager::ReservePrototypeContainer(_int iSceneCount)
{
	if (0 > iSceneCount)
		return E_FAIL;
	...
	{
		PRINT_LOG(L"Error", L"Failed To ReservePrototypeContainer");
		return E_FAIL;
	}
...
HRESULT CComponentManager::AddComponentPrototype(_int iSceneIndex,
												...
												CComponent* pPrototype)
{
	...
	{
		PRINT_LOG(L"Error", L"Out of range AddComponentPrototype");
		return E_FAIL;
	}
...
CComponent* CComponentManager::CloneComponentPrototype(_int iSceneIndex,
														const wstring& ComponentTag,
														void* pArg)
{
	if (0 > iSceneIndex ||
		m_iSceneCount <= iSceneIndex)
	{
		PRINT_LOG(L"Error", L"Out of range CloneComponentPrototype");
		return nullptr;
	}
...
HRESULT CComponentManager::ClearForScene(_int iSceneIndex)
{
	if (0 > iSceneIndex ||
		m_iSceneCount <= iSceneIndex)
	{
		PRINT_LOG(L"Error", L"Out of range ClearForScene");
		return E_FAIL;
	}
